...
export default function EquipmentHubPage() {
  return (
    <main className="bg-[#050505] text-white min-h-screen selection:bg-indigo-500 overflow-x-hidden">
      ...
      {/* Navigation & Status - Adjusted for Mobile Spacing */}
      <div className="pt-24 md:pt-32 px-4 md:px-6 max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <Link href="/" className="flex items-center gap-2 text-slate-400 hover:text-indigo-600 transition-colors font-bold text-[10px] md:text-xs uppercase tracking-widest">
          <ArrowLeft size={14} md={16} /> Back to Ecosystem
        </Link>
      ...
      {/* 2. THE THREE-WAY ECONOMY: Stackable Cards */}
      ...
      {/* 3. MARKET INTELLIGENCE: Vertical Stacking */}
      ...
      {/* 4. THE TYDEE SHIELD: Mobile Optimization */}
      <section className="py-20 md:py-32 px-4 md:px-6 bg-indigo-600 rounded-[2.5rem] md:rounded-[4rem]">
        <div className="max-w-7xl mx-auto text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-10 md:gap-12">
            <ShieldItem icon={<Video size={32}/>} title="AI Chain of Custody" desc="Mandatory video handoffs with AI damage detection." />
            <ShieldItem icon={<ShieldCheck size={32}/>} title="Micro-Insurance" desc="5% Protection Fee baked into every job." />
            <ShieldItem icon={<Zap size={32}/>} title="Biometric Staking" desc="High-value gear requires biometric verification." />
          </div>
        </div>
      </section>
...
function EcosystemCard({ icon, title, desc, tag }: any) {
  return (
    ...
    >
      <div className="mb-6 md:mb-8 p-3 md:p-4 bg-white rounded-xl md:rounded-2xl w-fit shadow-sm">{icon}</div>
...
function ShieldItem({ icon, title, desc }: any) {
  return (
    <div className="flex flex-col items-center text-white">
      <div className="bg-white/10 p-5 md:p-6 rounded-full mb-4 md:mb-6">{icon}</div>
...
function FeatureItem({ icon, title, desc }: any) {
  return (
    <div className="flex gap-4 md:gap-6 items-start">
      <div className="bg-indigo-500/10 p-2 md:p-3 rounded-lg md:rounded-xl text-indigo-400 shrink-0">{icon}</div>
      <div>
